...
bool CHARACTER::StartSlotMachine(uint8_t bBet)
{
	...
	int64_t iPrice = static_cast<int64_t>(SlotMachineBetMap[bBet]);
	if (GetGold() < iPrice)
	{
	...
	if (iFound >= ESlotMachine::MAX_SLOT_MACHINE_SLOTS)
	{
		...
		iWinnings = static_cast<uint64_t>(fTotal);
